...
const ProfileResultCard = ({ item }: { item: ILog }) => {
      ...
      <div className="flex sm:flex-row sm:grid sm:grid-cols-12 flex-col gap-4 w-full">
        <div className="flex flex-col sm:col-span-3 w-full">
          ...
          <span className="text-sm">
            {item.status === "done" &&
              item.profileDetail &&
              item.profileDetail.companies &&
              `${
                item.profileDetail.companies.filter(
                  (x) => x.status === "success"
                ).length
              }/${item.profileDetail.size} Found (${Math.round(
                (item.profileDetail.companies.filter(
                  (x) => x.status === "success"
                ).length /
                  item.profileDetail.size) *
                  100
              )}%)`}
          ...
          {item.status === "done" && (
            <>
              <span className="text-sm font-medium capitalize truncate w-full ml-1">
                ...
                item.profileDetail &&
                item.profileDetail.companies &&
                item.profileDetail.companies.length === 1
                  ? item.profileDetail.companies[0].name
                  : "Multiple Companies"}
              </span>
              <span className="text-sm font-medium ml-1">
                ...
                item.profileDetail.companies &&
                item.profileDetail.companies.length === 1
                  ? item.profileDetail.companies[0].url
                  : ""}
              </span>
            </>
          )}
        ...
        >
          {item.status === "done" &&
            ...
            item.profileDetail.companies.length === 1 && (
              <Link
                href={item.profileDetail.companies[0].url}
          ...
          {item.status === "done" &&
            ...
            item.profileDetail.companies && (
              <Link
                href={`/settings/profile/${item._id}`}
                rel="noopener noreferrer"
                target="_blank"
              >
                <Button variant="ghost" className="p-3">
