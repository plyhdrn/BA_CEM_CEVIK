...
import SetupLogCard from "./SetupLogCard";

const SetupLogWrapper = ({ logs, page }: { logs: ILog[]; page?: string }) => {
    ...
    <div>
      <div className="flex flex-col gap-2">
        <Fade duration={800} triggerOnce cascade damping={0}>
          {logs.map((item, i) => (
            <SetupLogCard key={item._id} item={item} />
          ))}
        </Fade>
      </div>
    </div>
...
export default SetupLogWrapper;
