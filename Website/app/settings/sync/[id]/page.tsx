...
const WebsiteFinderDetailPage = async ({
  params,
  searchParams,
}: {
  params: { id: string };
  ...
  const website = await Log.findOne({
    _id: id,
  });

  if (!website) return <Page404 />;
  ...
  const aggregate = [
    ...
    {
      $project: {
        companies: "$websiteDetail.companies",
      },
    },
    {
      $unwind: {
      ...
      <div className="container">
        <div className="flex flex-col justify-end py-8 gap-3">
          <div className="flex sm:flex-row flex-col sm:gap-0 gap-3 justify-between">
            <h1 className="text-4xl font-semibold">{website.name}</h1>
          </div>
          <div className="flex sm:flex-row flex-col gap-3 sm:gap-0 justify-between mt-4">
            <div className="flex flex-col gap-1">
              ...
              <span className="text-sm font-medium">
                {
                  website.websiteDetail.companies.filter(
                    (x) => x.status === "success"
                  ).length
                }
              </span>
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-sm font-medium">
                {convertTime(
                  (new Date(website.endDateTime).getTime() -
                    new Date(website.startDateTime).getTime()) /
                    1000
                )}
              </span>
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-sm font-medium">
                {new Date(website.createdAt).toString().slice(0, 24)}
              </span>
            </div>
          </div>
        </div>
...
export default WebsiteFinderDetailPage;
