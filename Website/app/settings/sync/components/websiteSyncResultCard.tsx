...
const WebsiteSyncResultCard = ({ item }: { item: ILog }) => {
      ...
      <div className="flex sm:flex-row sm:grid sm:grid-cols-12 flex-col gap-4 w-full">
        <div className="flex flex-col sm:col-span-3 w-full">
          ...
          <span className="text-sm">
            {item.status === "done" &&
              item.websiteDetail &&
              item.websiteDetail.companies &&
              `${
                item.websiteDetail.companies.filter(
                  (x) => x.status === "success"
                ).length
              }/${item.websiteDetail.size} Found (${Math.round(
                (item.websiteDetail.companies.filter(
                  (x) => x.status === "success"
                ).length /
                  item.websiteDetail.size) *
                  100
              )}%)`}
          ...
          {item.status === "done" && (
            <>
              <span className="text-sm font-medium capitalize truncate w-full ml-1">
                ...
                item.websiteDetail &&
                item.websiteDetail.companies &&
                item.websiteDetail.companies.length === 1
                  ? item.websiteDetail.companies[0].name
                  : "Multiple Companies"}
              </span>
              <span className="text-sm font-medium ml-1">
                ...
                item.websiteDetail.companies &&
                item.websiteDetail.companies.length === 1
                  ? item.websiteDetail.companies[0].url
                  : ""}
              </span>
            </>
          )}
        ...
        >
          {item.status === "done" &&
            ...
            item.websiteDetail.companies[0].url && (
              <Link
                href={item.websiteDetail.companies[0].url}
...
export default WebsiteSyncResultCard;
