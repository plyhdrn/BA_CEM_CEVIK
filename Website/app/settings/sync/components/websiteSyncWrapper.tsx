...
import WebsiteSyncResultCard from "./websiteSyncResultCard";

const WebsiteSyncWrapper = ({
  logs,
  page,
}: {
  logs: ILog[];
    ...
    <div>
      <div className="flex flex-col gap-2">
        <Fade duration={800} triggerOnce cascade damping={0}>
          {logs.map((item, i) => (
            <WebsiteSyncResultCard key={item._id} item={item} />
          ))}
        </Fade>
      </div>
    </div>
...
export default WebsiteSyncWrapper;
